...
function normalizeJsonSchema(jsonSchema) {
	// If schema has $ref and definitions, extract the main definition
	if (jsonSchema.$ref && jsonSchema.definitions) {
		const refPath = jsonSchema.$ref.replace('#/definitions/', '');
		const mainDefinition = jsonSchema.definitions[refPath];
		if (mainDefinition) {
			// Return the main definition with definitions attached
			return {
				...mainDefinition,
...
export function captureZodSchema(zodSchema, options = {}) {
	...
	try {
		...
		// Normalize the JSON schema structure
		...
		const metadata = extractSchemaMetadata(zodSchema, normalizedSchema);
		...
		const result = {
			...
			metadata,
			schemaHash: cacheKey,
			schemaVersion: generateSchemaVersion(normalizedSchema),
...
export function generateSchemaHash(schema) {
	try {
		// Extract the actual schema structure for hashing
		function extractSchemaStructure(obj) {
			if (obj === null || typeof obj !== 'object') {
				return obj;
			}
			
			// Handle Zod schema objects
			if (obj._def && obj._def.typeName) {
				const structure = {
					typeName: obj._def.typeName
				};
				
				// Extract shape for ZodObject
				if (obj._def.typeName === 'ZodObject' && typeof obj._def.shape === 'function') {
					const shape = obj._def.shape();
					structure.shape = extractSchemaStructure(shape);
				}
				
				// Extract other relevant _def properties
				if (obj._def.innerType) {
					structure.innerType = extractSchemaStructure(obj._def.innerType);
				}
				if (obj._def.type) {
					structure.type = extractSchemaStructure(obj._def.type);
				}
				if (obj._def.checks) {
					structure.checks = obj._def.checks;
				}
			...
			if (Array.isArray(obj)) {
				return obj.map(extractSchemaStructure);
			}
			
			const result = {};
			const keys = Object.keys(obj).sort();
			for (const key of keys) {
				result[key] = extractSchemaStructure(obj[key]);
			}
			return result;
		}
		
		const structure = extractSchemaStructure(schema);
		const schemaString = JSON.stringify(structure);
